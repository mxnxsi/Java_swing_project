...
public class TrainerReport extends JFrame implements ActionListener,MouseListener,DocumentListener{
    String username;
    JButton b1,b4,b2;
    JTextField t1;
    ...
    TrainerReport(String username){
        ...
        JLabel l3 = new JLabel("Joining Date :");
       ...
       // jdc.setMinSelectableDate(d);
        ...
        b2 = new JButton("All Trainer Details");
    ...
    public void actionPerformed(ActionEvent e){
           ...
           Conn c = new Conn();  
            try{
                if(!t1.getText().equals("")  && ((JTextField)jdc.getDateEditor().getUiComponent()).getText().equals("") && ((JTextField)jdc1.getDateEditor().getUiComponent()).getText().equals("")){
                    int count = 0;
                    ResultSet rs3 = c.s.executeQuery("select count(tID) from trainerinfo");
                    if(rs3.next()){
                        count = Integer.parseInt(rs3.getString("count(tID)"));
                    }    
                    if(Integer.parseInt(t1.getText()) > count){
                        ...
                        f.setVisible(true);
                        JasperDesign jd = JRXmlLoader.load("D:\\DAMS\\src\\dams\\reports\\trainerReport.jrxml");
                        String sql = "select * from trainerinfo where tid='"+t1.getText()+"'";
                ...
                if(t1.getText().equals("")  && !((JTextField)jdc.getDateEditor().getUiComponent()).getText().equals("") && ((JTextField)jdc1.getDateEditor().getUiComponent()).getText().equals("")){
                    ...
                    int count = 0;
                    ResultSet rs3 = c.s.executeQuery("select count(tfName) from trainerinfo where tjdate = '"+date1+"'");
                    if(rs3.next()){
                        count = Integer.parseInt(rs3.getString("count(tfName)"));
                    }    
                    if(count==0){
                        UIManager ui = new UIManager();
                        ui.put("OptionPane.background",Color.WHITE);
                        ...
                        f.setVisible(true);
                        JasperDesign jd = JRXmlLoader.load("D:\\DAMS\\src\\dams\\reports\\trainerReport.jrxml");
                        String sql = "select * from trainerinfo where tjdate = '"+date1+"'";
                        JRDesignQuery updatequery = new JRDesignQuery();
                        updatequery.setText(sql);
                        jd.setQuery(updatequery);
                        JasperReport jreport = JasperCompileManager.compileReport(jd);
                ...
                if(t1.getText().equals("")  && !((JTextField)jdc.getDateEditor().getUiComponent()).getText().equals("") && !((JTextField)jdc1.getDateEditor().getUiComponent()).getText().equals("")){
                    ...
                    int count = 0;
                    ResultSet rs3 = c.s.executeQuery("select count(tfName) from trainerinfo where tjdate >= '"+date1+"' && tjdate <= '"+date2+"'");
                    if(rs3.next()){
                        count = Integer.parseInt(rs3.getString("count(tfName)"));
                    }    
                    if(count==0){
                        ...
                        f.setVisible(true);
                        JasperDesign jd = JRXmlLoader.load("D:\\DAMS\\src\\dams\\reports\\trainerReport.jrxml");
                        String sql = "select * from trainerinfo where tjdate >= '"+date1+"' && tjdate <= '"+date2+"'";
                        JRDesignQuery updatequery = new JRDesignQuery();
                        updatequery.setText(sql);
                        jd.setQuery(updatequery);
                        JasperReport jreport = JasperCompileManager.compileReport(jd);
        ...
        if(e.getSource()==b2){
            ...
            try{
                JasperDesign jd = JRXmlLoader.load("D:\\DAMS\\src\\dams\\reports\\trainerReport.jrxml");
                String sql = "select * from trainerinfo";
                JRDesignQuery updateQuery = new JRDesignQuery();
                updateQuery.setText(sql);
                jd.setQuery(updateQuery);
                JasperReport jreport = JasperCompileManager.compileReport(jd);
